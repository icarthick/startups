...
_postconditions:
  ...
  - _assert: "all Validation Checklist items in clarify-assemble.md pass"
    _on_failure: _halt_and_inform
  - _assert: "if render-resource-inventory.json contains postgres services, then data.database_ha and global.migration_approach are both set (non-null)"
    _on_failure: _halt_and_inform
  - _assert: "if global.migration_approach is interim_cutover_data_first, then global.target_exit_date is non-null and a valid future date"
    _on_failure: _halt_and_inform
  - _assert: "if render-resource-inventory.json contains cron_job services, then operational.cron_target is set (non-null)"
    _on_failure: _halt_and_inform
_forbids_files:
  - README.md
  - "*.txt"
...
Questions are organized into **three batches** (≤7 per batch) presented sequentially. A standalone **fast-path** mode exists for simple stacks (< 4 services, no Postgres).
...
- Render-to-Fargate or Render-to-EB sizing decisions
