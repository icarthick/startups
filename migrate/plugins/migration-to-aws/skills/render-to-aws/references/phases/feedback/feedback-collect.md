...
```json
{
  ...
  "discovery": {
    "total_services": "<metadata.total_services_discovered>",
...
```json
{
  ...
  "cost_summary": {
    "current_render_monthly": "<source-side total or null>",
    "projected_aws_monthly": "<from estimation-infra.json>",
    "delta": "<projected - current or null>",
    "currency": "USD"
  },
  "detected_services": ["<service_type values from inventory>"],
...
**Encode**: Minify JSON → gzip → Base64URL. If > 8,192 chars, truncate `detected_services`, then longest `clarify_answers`. If still too large, skip.
