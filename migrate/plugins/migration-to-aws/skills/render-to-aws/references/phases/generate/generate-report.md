...
From design + preferences: region, primary compute (EB / Fargate),
service count.
...
| `exec-costs`       | Render-vs-AWS side-by-side when a Render baseline exists (`current_costs.source != "unavailable"`); otherwise the AWS three-tier table with a one-line note that no Render baseline was available. Estimated monthly; Balanced primary |
