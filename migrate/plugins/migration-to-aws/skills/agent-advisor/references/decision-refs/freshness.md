...
channel via WebFetch. Whatever cannot be verified this run stays cached and the footer must say so.

**WebFetch channel for Temporal-side facts:**
The `temporal-docs` MCP server has been removed. Use WebFetch to look up Temporal
documentation pages directly:

- Fetch `https://docs.temporal.io` for feature statuses and platform knowledge.
- For SDK-specific docs, fetch the relevant page under `https://docs.temporal.io/`.

**Procedure:**

1. Fetch the relevant `docs.temporal.io` page for the fact being verified.
2. If the fetch succeeds, use the returned value and list the field as verified.
3. If the fetch fails, use the cached value and list the field as fallen-back.
4. Only attempt once per fact per run; on failure, do not re-prompt.

- The anti-fabrication rule applies to WebFetch identically: a fact counts as verified
  only if the fetch actually returned it this run.
- Scope note: the Marketplace listing / commercial-terms fact below is AWS buyer-side
  and stays WebFetch-only — do not route it through any Temporal documentation source.
...
2. Attempt an aws-mcp server (`aws___get_regional_availability` / `aws___search_documentation`) lookup for each.
...
2. Refresh Registry availability via aws-mcp server (`aws___get_regional_availability` / `aws___search_documentation`) using the procedure above. A Runtime
...
- If some fields were MCP-verified this run:
  > _Generated `<DATE>`. Facts verified via AWS MCP Server: `<list verified>`. Cached values used
...
- If the MCP was not called / unavailable:
  > _Generated `<DATE>`. AWS MCP Server not called this run; all facts are cached values —
