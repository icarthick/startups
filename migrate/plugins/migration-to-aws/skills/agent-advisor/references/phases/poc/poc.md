...
`invocation_model_id`. A catalog date, an aws-mcp lookup, user acceptance, or a non-null
...
2. **`poc/deploy.sh`** — same guardrails as 3d (cost warning + typed `deploy` confirmation),
   ...
   aws-mcp (freshness rule); if not verified this run, write the command block with
...
- **Compute-type guard (read design.json first):** when the unit's
  ...
  CLI/API usage via the aws-mcp before writing that block (freshness rule).
...
- **The exact `agentcore` CLI flags are volatile** — verify current `configure`/`launch` usage
  via the aws-mcp (freshness rule). If not verified this run, keep the
...
# TODO: verify current agentcore CLI flags against AWS docs (aws-mcp)
