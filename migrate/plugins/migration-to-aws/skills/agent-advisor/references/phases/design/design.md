...
read the winning profile's `volatile_facts`, try aws-mcp server (`aws___get_regional_availability` / `aws___search_documentation`) for each, fall back to cached
...
(the `fedramp` volatile fact, via aws-mcp server (`aws___get_regional_availability` / `aws___search_documentation`)). Then surface an honest note: "AgentCore's
...
1. **Availability:** if the winning runtime is `agentcore` (or the chosen deployment model is
   Harness), verify it's available in the user's region via the aws-mcp server (`aws___get_regional_availability` / `aws___search_documentation`) (per
...
freshness.md's Temporal section defines the verification procedure — follow it exactly.
Use WebFetch to fetch the relevant `docs.temporal.io` page for each Temporal fact.
The `temporal-docs` MCP server has been removed; all Temporal lookups use WebFetch.
(The Marketplace listing fact also stays WebFetch-only.)
