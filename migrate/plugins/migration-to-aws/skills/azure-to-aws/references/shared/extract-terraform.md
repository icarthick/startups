...
`config.multiplicity_expression` set to the expression and
`config.multiplicity_resolved: false`. Do not fan out into N entries: the count is
usually a variable, so fanning out invents resources. Flag it, because a `for_each`
over a map of five apps is exactly the case where the estimate is otherwise five
times wrong in the other direction.
...
content is not in the workspace, add one `warnings[]` entry naming the module and
stating that its resources were not discovered — a silently missing module is a
