...
| **generate** | ✅ DONE — the MULTI-ARTIFACT phase: 2 fragments (terraform + docs) + a real cross-artifact validator assembler, + `templates/generate/**` (HCL/doc/script skeletons), `knowledge/generate/generate-routing.json`, `schemas/generation-warnings.schema.json`. New `templates/` taxonomy home (`_templates` key). Cold-LLM caught + fixed an apply-blocking dangling-SG defect; added reference-integrity gates. EKS-generate is a stub. Verdict in `test-output/generate-verdict.md`.       |
...
2. **All six phases are authored + cold-validated.** Remaining work:
   (a) the EKS cross-phase pass — author design's EKS branch (pod sizing +
   `eks_cluster` aggregate) + generate-eks (eks.tf + kubernetes/ manifests);
   estimate's EKS cost handling is already authored. One all-or-nothing port
   mirroring the MCP refactor's PR #85, with one end-to-end EKS cold-test.
   (b) optional: a CI validator implementing the conformance checklists in
   `docs/unit-taxonomy-spec.md`. (c) loose end: the clarify `migration_urgency`
   fast-path field (no schema property, untested) — add to schema or drop.
