...
# Generate — Stakeholder HTML Report (Render)

> Thin shareable HTML for SAs / founders. Decision + costs + optional what-if
> scenarios, then point at `MIGRATION_GUIDE.md` for procedure. Runs **after** docs
> so the guide exists when the report links to it.
...
| `render-resource-inventory.json` + `references/shared/render-pricing-cache.md`                                                 | Source-side Render costs — sum plan prices per service using the pricing cache as baseline; note that cache prices are estimates, not actuals                                    |
...
From `render-resource-inventory.json` + `references/shared/render-pricing-cache.md`:

- Sum the listed plan price for each service in the inventory using the pricing cache table (Web Services, Background Workers, Cron Jobs, Postgres, Key Value sections).
- Treat this as the **source-side baseline** (listed plan price, not metered actuals). Note in the report that Render bills metered overages (bandwidth, etc.) separately.
- If inventory has no priced services (e.g., all deferred or only out-of-scope types): omit the Render cost column and note unavailability.

From design + preferences: region, primary compute (EB / Fargate / Lambda), service count.
...
| `exec-costs`       | Render-vs-AWS side-by-side when a Render baseline is computable from the pricing cache; otherwise the AWS three-tier table with a one-line note that no Render baseline was available. Estimated monthly; Balanced primary             |
...
- Footer must include: `draft for review` and instruct readers to verify figures
  before sign-off.
- Title: `Render to AWS Migration Assessment`.
- Cost labeling: every dollar figure is an **estimated monthly** cost.
- Render cost note: label Render figures as "listed plan price (estimated)" — direct readers to https://render.com/pricing for current pricing and note that metered overages (bandwidth, etc.) are not included.
...
<head>
  ...
  <title>Render to AWS Migration Assessment</title>
  <style>/* inline */</style>
</head>
<body>
  <div class="report">
    ...
    <footer>Generated by Render to AWS Migration Advisor — draft for review; verify figures before executive sign-off.</footer>
  </div>
</body>
</html>
```
...
part of Generate for render-to-aws (stakeholder deliverable), but a report
failure should not delete Terraform/docs; repair the HTML and continue.

Optional (non-blocking): if
`$PLUGIN_ROOT/scripts/validate-render-migration-report.py` exists, run:

```
python3 scripts/validate-render-migration-report.py \
...
- Inventing cost figures not present in `estimation-infra.json` / scenario
  manifests / `render-pricing-cache.md`
