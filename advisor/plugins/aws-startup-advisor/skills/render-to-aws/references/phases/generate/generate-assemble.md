...
> `generate-docs.md`, `generate-report.md`) have written their artifacts. It runs the
> cross-artifact validation (every-service-generated-or-warned, reference integrity,
> no `{{VAR}}` leak), enforces the completion handoff gate, and updates
> `.phase-status.json`. It owns the phase's final artifact-level contract.
...
- If any service has `aws_service: "Fargate"` (non-cron) → `terraform/fargate.tf` must exist
- If any service has `aws_service: "Lambda"` → `terraform/cron.tf` must exist
- If any service has `aws_service: "RDS PostgreSQL"` or `aws_service: "Aurora PostgreSQL"` → `terraform/rds.tf` must exist
- If any service has `aws_service: "ElastiCache Redis"` → `terraform/elasticache.tf` must exist
...
sections, report sections, conditional Postgres/scripts, conditional EB/Fargate/Lambda
terraform, every service accounted for, no `{{VARIABLE}}` placeholders), then emit
`GATE_FAIL` (STOP) or
`HANDOFF_OK | phase=generate | artifacts=terraform/,MIGRATION_GUIDE.md,README.md,migration-report.html`.

Optionally run
`python3 "$PLUGIN_ROOT/scripts/validate-render-migration-report.py" \
  "$MIGRATION_DIR/migration-report.html" --migration-dir "$MIGRATION_DIR"`
(if the script exists) and treat exit `1` as `GATE_FAIL` for the report (repair HTML; do not delete
Terraform/docs).
...
• scripts/ — Database migration scripts (when Postgres is in scope)
...
This phase's artifacts are declared in `_produces` (the terraform floor + MIGRATION_GUIDE.md + README.md + generation-warnings.json; `.phase-status.json` is updated per Step 4). Conditional outputs (`scripts/migrate-postgres.sh` when Postgres in design) are governed by the terraform fragment's Step 5; `generation-warnings.json` is always written (empty `warnings` array when nothing was skipped).
