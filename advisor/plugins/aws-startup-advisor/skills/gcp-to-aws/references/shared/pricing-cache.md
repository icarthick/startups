...
~15–20% below the x86 equivalent at the same vCPU/memory. Dev-tier rows cached below; query the `aws-mcp` server for any family or size not listed.
...
> server documentation lookup returns no pricing rows for them, and an empty result is **not** evidence the model is unavailable. (An earlier revision also
