...
| `google_artifact_registry_repository`       | ECR                  | Always     | ECR is OCI/Docker only; AR also supports Maven, npm, Python, Go, Helm — split non-container formats to CodeArtifact. Auth: use `aws ecr get-login-password` + IAM (vs GCP IAM-based `gcloud auth`). Per-format repos in AR → one ECR repo per image family. |
