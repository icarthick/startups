...
- **The migration skills use MCP servers for live data** declared in the plugin's `.mcp.json`: `awsknowledge` (HTTP), `awspricing` (stdio via `uvx`), and `aws-pricing-calculator` (stdio via `npx`). They are enhancements, not hard dependencies — the skills run without them, with pricing falling back to a bundled cache and docs lookups skipped. The AWS Pricing server needs `uv`/`uvx` on the machine.
